...
# PITCH_DETECTION_MODEL_PATH = os.path.join(PARENT_DIR, 'data/football-pitch-detection-mike_640.pt')
PITCH_DETECTION_MODEL_PATH = os.path.join(PARENT_DIR, 'data/football-pitch-detection-mike_640_v11m.pt')
BALL_DETECTION_MODEL_PATH = PLAYER_DETECTION_MODEL_PATH
...
def performanceMeter(desc: str = ''):
    ...
    # print(f'Time taken: {time.perf_counter() - startT:.2f} seconds | {desc}')
    # startT = time.perf_counter()
...
def render_radar(
    ...
    # print('render_radar keypoints', keypoints)
...
def run_radar(source_video_path: str, device: str) -> Iterator[np.ndarray]:
    ...
    nrCropsToTrain = 30
    print('collecting', nrCropsToTrain, 'crops to train')
    # for frame in tqdm(frame_generator, desc='collecting crops'):
    frameI = 0
    for frame in frame_generator:
        frameI+=1
        # if frameI > nrCropsToTrain:
        #     print('breaking crop collection')
        #     break
        result = player_detection_model(frame, imgsz=1280, verbose=False)[0]
        detections = sv.Detections.from_ultralytics(result)
        teamColorTrainingCrops+= get_crops(frame, detections[detections.class_id == PLAYER_CLASS_ID])

    performanceMeter('collecting teamColorTrainingCrops from all frames')

    team_classifier = TeamClassifier(device=device)
    team_classifier.fit(teamColorTrainingCrops)
    ...
    for frame in frame_generator:
        ...
        crops = get_crops(frame, players)
        players_team_id = team_classifier.predict(crops)
        # print('now detections tracker id', detections.tracker_id)
        # print('outsideTrackerIds_Team loop', outsideTrackerIds_Team)
        # players_team_id = [round(statistics.mean(outsideTrackerIds_Team[str(tracker_id)])) for tracker_id in players.tracker_id]

        for i in range(len(players.tracker_id)):
            if str(players.tracker_id[i]) not in insideTrackerIds_Team:
                insideTrackerIds_Team[str(players.tracker_id[i])] = []
            insideTrackerIds_Team[str(players.tracker_id[i])].append(int(players_team_id[i]))

        players_team_id = [round(statistics.mean(insideTrackerIds_Team[str(tracker_id)])) for tracker_id in players.tracker_id]

        players_team_id = np.array(players_team_id)
        ...
        # goalkeepers_team_id = resolve_goalkeepers_team_id(players, players_team_id, goalkeepers)
        referees = detections[detections.class_id == REFEREE_CLASS_ID]
        balls = originalDetections[originalDetections.class_id == BALL_CLASS_ID]
        ...
        # print(f'Time taken: {time.perf_counter() - frameStartT:.2f} seconds | Total frame time | {1 / (time.perf_counter() - frameStartT):.2f} fps')
