...
BALL_DETECTION_MODEL_PATH = os.path.join(PARENT_DIR, 'data/football-ball-detection_mike.pt')
...
def run_radar(source_video_path: str, device: str) -> Iterator[np.ndarray]:
    ...
    performanceMeter('collecting crops from all frames')
    ...
    tempTeamIds = []
    for frame in frame_generator:
        tempResult = player_detection_model(frame, imgsz=640, verbose=False)[0]
        ...
        # print('_tempDetections vs tempCropsWithTracker vs _tempTeamIds len:', len(_tempDetections), len(tempCrops), len(tempCropsTracker), len(_tempTeamIds))

    for i in range(len(tempDetections)):
        # print('tempDetections[i][0]',tempDetections[i])
        # print('tempDetections[i].tracker_id',tempDetections[i].tracker_id)
        for j in range(len(tempDetections[i].tracker_id)):
            if str(tempDetections[i].tracker_id[j]) not in trackerIds_Team:
                trackerIds_Team[str(tempDetections[i].tracker_id[j])] = []
            trackerIds_Team[str(tempDetections[i].tracker_id[j])].append(int(tempTeamIds[i][j]))
    print('trackerIds_Team', trackerIds_Team)
    ...
    for frame in frame_generator:
        ...
        print('trackerIds_Team loop', trackerIds_Team)
        players_team_id = [round(statistics.mean(trackerIds_Team[str(tracker_id)])) for tracker_id in players.tracker_id]
