...
from main_1 import PROJECT_ROOT
...
PLAYER_DETECTION_MODEL_PATH = os.path.join(PROJECT_ROOT, 'models', 'active', 'yolo12l_jogadores_bola_1.pt')
...
BALL_DETECTION_MODEL_PATH = PLAYER_DETECTION_MODEL_PATH     # player model already detects the ball well enough
...
def run_radar(source_video_path: str, device: str) -> Iterator[np.ndarray]:
    ...
    # for frame in tqdm(frame_generator, desc='collecting crops'):
    MAX_TRAIN_DETECTIONS = 100
    for frame in frame_generator:
        if len(teamColorTrainingCrops) > MAX_TRAIN_DETECTIONS:
            print('reached', MAX_TRAIN_DETECTIONS, 'frames: stopping')
            break
        result = player_detection_model(frame, imgsz=1280, verbose=False)[0]
        detections = sv.Detections.from_ultralytics(result)
    ...
    # classified_crops = team_classifier.predict(teamColorTrainingCrops)
    ...
    for frame in frame_generator:
        ...
        # players_team_id = [round(statistics.mean(insideTrackerIds_Team[str(tracker_id)][-75:])) for tracker_id in players.tracker_id]
