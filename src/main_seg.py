...
def run_ball_detection(source_video_path: str, device: str) -> Iterator[np.ndarray]:
    ...
    def callback(image_slice: np.ndarray) -> sv.Detections:
        result = ball_detection_model(image_slice, imgsz=1024, half=True, conf=0.30, verbose=False)[0]
...
def run_team_classification(source_video_path: str, device: str, debug: bool = False) -> Iterator[np.ndarray]:
    ...
    for frame in frame_generator:
        ...
        # Extrair máscaras de segmentação (shape: [N, H_mask, W_mask])
        all_masks = result.masks.data.cpu().numpy() if result.masks is not None else None

        # Filter out detections without tracker IDs — propagar filtro às masks
        if detections.tracker_id is not None:
            valid_mask_bool = detections.tracker_id != -1
            detections = detections[valid_mask_bool]
            if all_masks is not None:
                all_masks = all_masks[valid_mask_bool]
    ...
    for frame in frame_generator:
        ...
        if last_detections is None:  # first frame
            ...
            if len(detections) == 0:
                ...
                all_masks = None  # sem deteções novas, sem máscaras novas
            else:
                last_detections = detections
        ...
        if last_keypoints is None:  # first frame
            ...
            if len(keypoints.xy) == 0:
                ...
                if np.count_nonzero(keypoints.xy[0]) < 8:
                    keypoints = last_keypoints
                else:
                    last_keypoints = keypoints
